...
/***************************** */
    ...
    if (Config.CookieArray?.length > 0) {
        ...
        changetime++;
        const cookieModelLog = /(claude[-_][a-z0-9-_]*?)@(?:sessionKey=)?sk-ant-sid01-[\w-]{86}-[\w-]{6}AA/.exec(Config.Cookie)?.[1];
        if (model && cookieModelLog && cookieModelLog != 'claude_pro' && cookieModelLog != model) return CookieChanger(false);
    }
...
/**************************** */
    ...
    if (accInfo?.active_flags.length > 0) {
        ...
        return banned ? CookieCleaner() : Config.Settings.SkipRestricted && Config.CookieArray > 1 && CookieChanger(); //
    }
...
/******************************** */
                ...
                const shouldChange = Config.CookieArray?.length > 0 && (exceeded_limit || !nochange && Config.Cookiecounter > 0 && changeflag++ >= Config.Cookiecounter - 1); //
